...
class IncrementalKDE:
        ...
        def update(self, new_goals):
            if self.goals is None:
                ...
                self.goals = np.vstack((self.goals, new_goals))
            self.kde.fit(self.goals)

        def sample(self, sample_size):
            log_density = self.kde.score_samples(self.goals)
            density = np.exp(log_density)
            min_density_indices = np.argsort(density)[:sample_size]
            return self.goals[min_density_indices]
...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def _sample_goals_with_kde_2(self, batch_indices: np.ndarray, env_indices: np.ndarray, kde_sample_size: int = 30,
                            recent_size: int = 3000, exploration_factor: float = 0.5) -> np.ndarray:
        # Get the indices of recent transitions
        recent_indices = np.arange(max(0, self.pos - recent_size), self.pos)
        # Gather the recent achieved goals
        recent_goals = self.next_observations["achieved_goal"][recent_indices, :, :].reshape(-1, self.next_observations[
            "achieved_goal"].shape[-1])
        # Perform grid search to find the optimal bandwidth
        params = {'bandwidth': np.logspace(-1, 1, 20)}
        grid = GridSearchCV(KernelDensity(), params, cv=5)
        grid.fit(recent_goals)
        best_bandwidth = grid.best_estimator_.bandwidth
        # Fit KDE model on the recent goals with the optimal bandwidth
        kde = KernelDensity(kernel='gaussian', bandwidth=best_bandwidth).fit(recent_goals)
        log_density = kde.score_samples(recent_goals)
        density = np.exp(log_density)
        # Normalize density estimates to sum up to 1
        density = density / np.sum(density)
        # Sample goals based on the exploration-exploitation trade-off
        if np.random.rand() < exploration_factor:
            # Exploration: Sample goals from low-density regions
            sampled_goals_idx = np.random.choice(len(recent_goals), size=kde_sample_size, replace=False, p=1 - density)
        else:
            # Exploitation: Sample goals from high-density regions
            sampled_goals_idx = np.random.choice(len(recent_goals), size=kde_sample_size, replace=False, p=density)

        sampled_goals = recent_goals[sampled_goals_idx]
        sampled_goals_idx = np.random.choice(range(len(sampled_goals)), size=len(batch_indices), replace=True)
        sampled_goals = sampled_goals[sampled_goals_idx]

        return sampled_goals

    # def _sample_goals_with_weighted_kde(self, batch_indices: np.ndarray, env_indices: np.ndarray, kde_sample_size: int = 30,
    #                                 recent_size: int = 3000) -> np.ndarray:
    #     recent_indices = np.arange(max(0, self.pos - recent_size), self.pos)
    #     recent_goals = self.next_observations["achieved_goal"][recent_indices, :, :].reshape(-1, self.next_observations["achieved_goal"].shape[-1])

    #     # Calculate weights based on the distance to the desired goal
    #     weights = 1.0 / (np.linalg.norm(recent_goals - self.desired_goal, axis=-1) + 1e-6)
    #     weights /= np.sum(weights)

    #     kde = KernelDensity(kernel='gaussian', bandwidth=0.5).fit(recent_goals, sample_weight=weights)
    #     log_density = kde.score_samples(recent_goals)
    #     density = np.exp(log_density)

    #     min_density_indices = np.argsort(density)[:kde_sample_size]
    #     sampled_goals = recent_goals[min_density_indices]
    #     sampled_goals_idx = np.random.choice(range(len(sampled_goals)), size=len(batch_indices), replace=True)
    #     sampled_goals = sampled_goals[sampled_goals_idx]

    #     return sampled_goals
    
    def _sample_goals_with_weighted_kde(self, batch_indices: np.ndarray, env_indices: np.ndarray, desired_goal: np.ndarray, kde_sample_size: int = 30,
                                    recent_size: int = 3000) -> np.ndarray:
        recent_indices = np.arange(max(0, self.pos - recent_size), self.pos)
        recent_goals = self.next_observations["achieved_goal"][recent_indices, :, :].reshape(-1, self.next_observations["achieved_goal"].shape[-1])

        # Calculate weights based on the distance to the desired goal
        weights = 1.0 / (np.linalg.norm(recent_goals - desired_goal, axis=-1) + 1e-6)
        weights /= np.sum(weights)
    ...
    def _sample_goals_with_incremental_kde(self, batch_indices: np.ndarray, env_indices: np.ndarray, kde_sample_size: int = 20,
                                        recent_size: int = 1000) -> np.ndarray:
        recent_indices = np.arange(max(0, self.pos - recent_size), self.pos)
        recent_goals = self.next_observations["achieved_goal"][recent_indices, :, :].reshape(-1, self.next_observations["achieved_goal"].shape[-1])

        if not hasattr(self, 'incremental_kde'):
            self.incremental_kde = IncrementalKDE()

        self.incremental_kde.update(recent_goals)
        sampled_goals = self.incremental_kde.sample(kde_sample_size)
        sampled_goals_idx = np.random.choice(range(len(sampled_goals)), size=len(batch_indices), replace=True)
        sampled_goals = sampled_goals[sampled_goals_idx]
    ...
    def _sample_goals_with_partitioning(self, batch_indices: np.ndarray, env_indices: np.ndarray, kde_sample_size: int = 20,
                                        recent_size: int = 1000, num_partitions: int = 5) -> np.ndarray:
        recent_indices = np.arange(max(0, self.pos - recent_size), self.pos)
        recent_goals = self.next_observations["achieved_goal"][recent_indices, :, :].reshape(-1, self.next_observations["achieved_goal"].shape[-1])

        kmeans = KMeans(n_clusters=num_partitions).fit(recent_goals)
        labels = kmeans.labels_

        sampled_goals = []
        for i in range(num_partitions):
            partition_goals = recent_goals[labels == i]
            kde = KernelDensity(kernel='gaussian', bandwidth=0.5).fit(partition_goals)
            log_density = kde.score_samples(partition_goals)
            density = np.exp(log_density)
            min_density_indices = np.argsort(density)[:kde_sample_size // num_partitions]
            sampled_goals.append(partition_goals[min_density_indices])

        sampled_goals = np.concatenate(sampled_goals)
        sampled_goals_idx = np.random.choice(range(len(sampled_goals)), size=len(batch_indices), replace=True)
        sampled_goals = sampled_goals[sampled_goals_idx]

        return sampled_goals

    def _sample_goals_with_kde_1(self, batch_indices: np.ndarray, env_indices: np.ndarray, kde_sample_size: int = 30,
                            recent_size: int = 3000) -> np.ndarray:
        """
        Sample goals based on the lowest density regions of a KDE model fitted on recent goals.

        :param batch_indices: Indices of the transitions
        :param env_indices: Indices of the kde
        :param kde_sample_size: Number of goals to sample from the lowest density regions
        :param recent_size: Number of recent goals to use for KDE fitting
        ...
        # Fit KDE model on the recent goals
        kde = KernelDensity(kernel='Gaussian', bandwidth=0.5).fit(recent_goals)
        log_density = kde.score_samples(recent_goals)
        density = np.exp(log_density)

        min_density_indices = np.argsort(density)[:kde_sample_size]

        # Sample goals based on lowest density
        sampled_goals = recent_goals[min_density_indices]
        sampled_goals_idx = np.random.choice(range(len(sampled_goals)), size=len(batch_indices), replace=True)
        sampled_goals = sampled_goals[sampled_goals_idx]
        # print(len(sampled_goals))

        return sampled_goals

    def _sample_goal_with_central_derivative(self, batch_indices: np.ndarray) -> np.ndarray:
        sampled_goals = np.array([(0.031249999999999993, 0.5), (0.05468749999999999, 0.45940505919760444), (0.1015625, 0.3782151775928133), (0.1484375, 0.45940505919760444), (0.1953125, 0.21583541438323106), (0.1953125, 0.3782151775928133), (0.1953125, 0.5405949408023956), (0.1953125, 0.7841645856167689), (0.21875, 0.33762023679041775), (0.21875, 0.8247595264191645), (0.2421875, 0.13464553277843994), (0.265625, 0.09405059197604437), (0.265625, 0.9059494080239556), (0.2890625, 0.3782151775928133), (0.3125, 0.09405059197604437), (0.3125, 0.33762023679041775), (0.3125, 0.9059494080239556), (0.3828125, 0.2970252959880222), (0.40625, 0.9059494080239556), (0.453125, 0.09405059197604437), (0.453125, 0.9059494080239556), (0.5, 0.9059494080239556), (0.5234375, 0.3782151775928133), (0.546875, 0.9059494080239556), (0.5703125, 0.8653544672215601), (0.6171875, 0.7841645856167689), (0.640625, 0.09405059197604437), (0.640625, 0.4188101183952089), (0.6640625, 0.13464553277843994), (0.7109375, 0.21583541438323106), (0.7109375, 0.45940505919760444), (0.734375, 0.9059494080239556), (0.7578125, 0.8653544672215601), (0.78125, 0.8247595264191645), (0.8046875, 0.21583541438323106), (0.8046875, 0.7841645856167689), (0.8515625, 0.45940505919760444), (0.875, 0.33762023679041775), (0.875, 0.6623797632095823), (0.8984375, 0.45940505919760444), (0.9453125, 0.45940505919760444), (0.96875, 0.5)])
        sampled_goals_idx = np.random.choice(range(len(sampled_goals)), size=len(batch_indices), replace=True)
        sampled_goals = sampled_goals[sampled_goals_idx]
        return sampled_goals
    ...
    def _sample_goals(self, batch_indices: np.ndarray, env_indices: np.ndarray) -> np.ndarray:
        """
        Sample goals based on goal_selection_strategy or using a KDE model.
        ...
        if self.goal_selection_strategy == GoalSelectionStrategy.FINAL:
            ...
            transition_indices_in_episode = self.ep_length[batch_indices, env_indices] - 1

        elif self.goal_selection_strategy == GoalSelectionStrategy.FUTURE:
            # Replay with random state which comes from the same episode and was observed after current transition
            # Note: our implementation is inclusive: current transition can be sampled
            current_indices_in_episode = (batch_indices - self.ep_start[batch_indices, env_indices]) % self.buffer_size
            transition_indices_in_episode = np.random.randint(current_indices_in_episode,
                                                              self.ep_length[batch_indices, env_indices])

        elif self.goal_selection_strategy == GoalSelectionStrategy.EPISODE:
            # Replay with random state which comes from the same episode as current transition
            transition_indices_in_episode = np.random.randint(0, self.ep_length[batch_indices, env_indices])

        elif self.goal_selection_strategy == GoalSelectionStrategy.KDE:
            # Sample goals from low-density regions of a KDE model
            ######################################################
            ######################################################
            ######################################################
            # return self._sample_goals_with_kde_2(batch_indices, env_indices)
            
            return self._sample_goals_with_kde_1(batch_indices, env_indices)
            
            # obs_dict = self.env.reset()
            # desired_goal = obs_dict['desired_goal'][0]
            # return self._sample_goals_with_weighted_kde(batch_indices, env_indices, desired_goal)
            
            # return self._sample_goals_with_incremental_kde(batch_indices, env_indices)
            # return self._sample_goals_with_partitioning(batch_indices, env_indices)
            ######################################################
            ######################################################
            ######################################################

        elif self.goal_selection_strategy == GoalSelectionStrategy.CEN:
            return self._sample_goal_with_central_derivative(batch_indices)

        else:
            raise ValueError(f"Strategy {self.goal_selection_strategy} for sampling goals not supported!")

        transition_indices = (transition_indices_in_episode + self.ep_start[
            batch_indices, env_indices]) % self.buffer_size
